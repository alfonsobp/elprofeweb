...
class LocalizationController extends BaseController {
	...
	public function getDepartments() {
		...
		$localizations_depts = Localization::where('cod_prov','=','00')->where('cod_dist','=','00')->get(array('cod_dept','name'));
		return $localizations_depts;
	}

	public function getProvinces($cod_dept) {
		$localizations_provs = Localization::where('cod_dept','=',$cod_dept)->where('cod_prov','<>','00')->where('cod_dist','=','00')->get(array('cod_prov','name'));
		return Response:: json($localizations_provs);	
	}

	public function getDistricts($cod_dept,$cod_prov) {
		$localizations_dists = Localization::where('cod_dept','=',$cod_dept)->where('cod_prov','=',$cod_prov)->where('cod_dist','<>','00')->get(array('cod_dist','name'));
